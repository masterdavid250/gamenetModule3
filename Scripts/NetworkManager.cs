...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    public RacingLineup[] racingChallengers;
    public DeathRaceLineup[] deathRaceChallengers;
    ...
    public void OnJoinRandomRoomClick(string gameMode)
    {
        GameMode = gameMode;
    ...
    public override void OnJoinedRoom()
    {
        ...
        if (PhotonNetwork.CurrentRoom.CustomProperties.TryGetValue("gm", out gameModeName))
        {
            ...
            if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("rc"))
            {
                ...
                {
                    CarPool[i].transform.Find("PlayerName").GetComponent<Text>().text = racingChallengers[i].playerName;
                    CarPool[i].transform.Find("PlayerProperty").GetComponent<Text>().text = "";
                }
            }
            else if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsValue("dr"))
            {
                ...
                for (int i = 0; i < CarPool.Length; i++)
                {
                    CarPool[i].transform.Find("PlayerName").GetComponent<Text>().text = deathRaceChallengers[i].playerName;
                    CarPool[i].transform.Find("PlayerProperty").GetComponent<Text>().text = deathRaceChallengers[i].weaponName; 
                }
            }
        }
        ...
        foreach (Player player in PhotonNetwork.PlayerList)
        {
            ...
            playerListItem.GetComponent<PlayerListInitializer>().Initialize(player.ActorNumber, player.NickName);

            object isPlayerReady; 
            if (player.CustomProperties.TryGetValue(Constants.PLAYER_READY, out isPlayerReady))
            {
                playerListItem.GetComponent<PlayerListInitializer>().SetPlayerReady((bool) isPlayerReady);
            }
    ...
    public override void OnPlayerEnteredRoom(Player newPlayer)
    {
        ...
        playerListItem.GetComponent<PlayerListInitializer>().Initialize(newPlayer.ActorNumber, newPlayer.NickName);
    ...
    public override void OnPlayerPropertiesUpdate(Player targetPlayer, ExitGames.Client.Photon.Hashtable changedProps)
    {
        ...
        if (playerListGameObjects.TryGetValue(targetPlayer.ActorNumber, out playerlistGameObject))
        {
            ...
            {
                playerlistGameObject.GetComponent<PlayerListInitializer>().SetPlayerReady((bool) isPlayerReady);
            }
        }
